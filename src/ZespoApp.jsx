...
const MOD_PROFILE = {
  ...
  bio: "Keeper of the access matrix. Handles role requests and anything broken.",
  pfp: "🛡️",
};
...
export default function ZespoApp() {
  ...
  useEffect(() => {
    api.getSession()
      .then((s) => { setSession(s); setAuthChecked(true); })
      .catch((e) => {
        setConnError(e?.message || "Couldn't reach Supabase. Check your connection and try again.");
        setAuthChecked(true);
      });
      ...
      >
        ...
        {stage === "auth" && (
          <div key="auth" className="flex-1 flex flex-col min-h-0 anim-fadeUp">
            <AuthScreen theme={theme} connError={connError} />
          </div>
        )}
...
function AuthScreen({ theme, connError }) {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-y-auto scrollbar-thin px-6 pt-10 pb-8" style={{ background: `radial-gradient(circle at 50% -10%, ${theme.accent}22, transparent 60%)` }}>
      ...
      {connError && (
        <div className="mb-6 rounded-xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-xs text-red-300 font-mono2">
          Can't reach the server: {connError} — check your connection and reload the page.
        </div>
      )}
...
function ModeratorTab({ theme, currentUser, pushAnnouncement }) {
  const [log, setLog] = useState([
    { from: "bot", text: `Hey @${currentUser.username}. I'm the moderator bot — I can point you to how roles work, or log a custom issue for the team. What do you need?` },
  ]);
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="px-4 py-3 border-t border-white/5 flex gap-2 flex-wrap">
        {["Role selection", "Custom issue"].map(o => (
